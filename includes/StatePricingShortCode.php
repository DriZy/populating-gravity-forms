...
class StatePricingShortCode {
    ...
    public function state_pricing_iniatilise_array(){
        ...
        if ($xlsx = SimpleXLSX::parse($file_path)) {
            if (count($xlsx->rows()) < 2) {
                echo 'The excel file has no data <br>';
                return;
            } else {
                $count = $xlsx->rows();
                foreach ($xlsx->rows() as $key => $column) {
                    if ($key != 0) {
                        if (array_key_exists(trim($column[0]), $datas)) {
                            array_push($datas[trim($column[0])], [trim($column[0]), $column[1], $column[2], $column[3]]);
                        } else {
                            $datas[trim($column[0])] = array();
                            array_push($datas[trim($column[0])], [trim($column[0]), $column[1], $column[2], $column[3]]);
                        }
                    }
                }
    ...
    function license_pricing_get_values(){
        ...
        if(array_key_exists($state, $datas)){
        	...
        	$response['false'] = 'Sorry the state enter do not match any result';
        }
        echo json_encode(array_unique($response));
        die();
    }
